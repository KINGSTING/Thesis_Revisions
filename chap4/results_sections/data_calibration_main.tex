...
\begin{table}[H]
    ...
    \begin{tabular}{l r r c c c c}
        ...
        Brgy Esperanza    & 574   & 90,000  & 14\% & 20 & 50 & 30 \\
...
The parameterization process identified three distinct socioeconomic typologies that fundamentally altered the marginal utility of money within the agents' decision-making algorithms. The first typology, classified as the ``Urban Resource Trap'' and exemplified by Barangay Poblacion, presents a significant hurdle for the DRL agent. Despite possessing the highest absolute local budget (\textpeso 200,000), Poblacion suffers from the lowest per-capita spending power due to its massive population density ($N=1,534$). Furthermore, because this jurisdiction is bound by its historical ``Police State'' profile, 60\% of its internal budget is pre-allocated to enforcement simply to maintain basic community order. This institutional constraint mathematically caps its baseline autonomous compliance at an abysmal 2\%, creating a high-inertia environment that resists small-scale interventions.

Conversely, Barangay Binuni represents a ``Wealthy Anomaly'' within the municipal landscape. Operating with a robust \textpeso 126,370 budget for only 507 households, Binuni enjoys the highest per-capita fiscal surplus in the model. This demographic wealth generates a highly elastic response to policy, as the barangay possesses sufficient liquidity to overcome behavioral friction through consistent local incentives. From an optimization standpoint, Binuni serves as a low-resistance node where compliance can be achieved with relatively high efficiency compared to its neighbors.

Finally, the ``Subsistence Trap'' observed in Babalaya and Demologan defines the economic floor of the municipality. In these zones, 80\% of households are classified as low-income, making them highly sensitive to financial penalties but less responsive to purely educational outreach. Operating on micro-budgets ranging from \textpeso 15,000 to \textpeso 21,000, up to 90\% of their internal funds are consumed by bare-minimum enforcement honoraria. Computationally, these barangays represent ``dead zones'' in the state space where autonomous IEC and incentive distributions are mathematically impossible. The simulation demonstrates that these communities remain in a state of permanent compliance stagnation unless the Mayor Agent provides direct municipal augmentation, highlighting the critical role of top-down fiscal intervention in achieving municipal-wide equity.
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c c c c c c c c}
        ...
        Esperanza    & 0.20 & 0.35 & 0.25 & 0.65 & 0.0015 & & 50 & 30 & 20 \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c p{6.5cm}}
        ...
        Brgy Esperanza    & 20\%  & 14.0\% & Aligned with the middle-class control group variance. \\
        Brgy Poblacion    & 2\%   & 2.0\%  & \textbf{Anchor Point} (Aligned perfectly with MENRO Audit). \\
        \bottomrule
    \end{tabular}
\end{table}

To reconcile the remaining micro-level bias with the macro-level ground truth, the initialized states for the non-anchor barangays were assigned stochastically using a bounded uniform distribution, $U(0.11, 0.15)$. The mathematical expected value ($\mu$) of this distribution tightly anchors to the 12.5\% MENRO baseline. By executing this randomization, headless simulation benchmarks successfully stabilized at a Global Average Compliance of $\approx 12.5\%$ at $t = 0$. This verified the structural validity of the ABM, ensuring the subsequent DRL agent was optimizing against a rigorously realistic, noisy scenario rather than a mathematically trivial or perfectly uniform one \citep{McCulloch2022}.

\subsection{The ``Act vs. Reality'' Gap and the Danger of Algorithmic Bias}
The systemic necessity of the aforementioned calibration phase exposes a profound sociological and computational challenge in municipal governance: the pervasive danger of Socially Desirable Responding (SDR) \citep{Geiger2021}. The data extracted during the initialization phase quantitatively captured the ``Act vs. Reality'' gap described by \cite{Yazawa2025}, illustrating a significant trend where local officials conflate theoretical policy awareness with actual, physical execution on the ground. From a machine learning perspective, addressing this statistical divergence was paramount to preventing algorithmic bias. Had the simulation trusted the unverified self-reports—such as the near-perfect compliance reported in Binuni or Babalaya—the RL environment would have suffered from a ``garbage in, garbage out'' (GIGO) failure. In such a scenario, the AI Mayor would have been trained on a trivialized state space, systematically learning to underfund or ignore crucial sectors based on the mathematically false assumption that they had already achieved permanent environmental sustainability.

The analytical pivots required to reconcile these figures expose three critical administrative blind spots that frequently cause real-world policies to fail. The first of these, identified in Barangay Binuni, is the Intention-Action Trap. As empirical evidence suggests, high positive attitudes toward the environment frequently coexist with poor R.A. 9003 compliance. When human administrators treat verbal support for environmental initiatives as equivalent to physical segregation, they are often misled into prematurely withdrawing municipal funding or enforcement presence before those habits are truly habituated. This overestimation of compliance is mirrored in the Transient Project Trap observed in Liangan East. Here, temporary spikes in compliance were driven by specific, transactional initiatives—such as the barter of Eco-bricks for rice—rather than deeply held social norms. The calibration process corrects the dangerous assumption that such behavior will persist autonomously; in reality, once the external funding for bartering vanishes, the behavior exhibits a rapid decay ($\delta$) back to the historical baseline.

Furthermore, the simulation's calibration reveals a stark Urban Resource Trap, exemplified by the commercial center of Barangay Poblacion. Strikingly, Poblacion was the only jurisdiction where qualitative reporting matched the objective quantitative audit, reflecting an abysmal 2\% compliance rate. This alignment supports the sociological theory that high population density and urban anonymity actively dissolve the social pressure required to maintain community standards, creating an environment that defaults to non-compliance despite the existence of legislative frameworks \citep{Villanueva2021}. Ultimately, correcting these statistical divergences indicates that a successful smart governance framework cannot rely on subjective, politically motivated municipal reporting. To successfully bridge the gap between legislative intent and physical action, the AI's optimization logic—and by extension, the LGU's real-world policy design—must be anchored strictly to objective, systemic ground-truths.