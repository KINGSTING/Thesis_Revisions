...
\subsection{Architectural Shift: From Offline to Interactive Online DRL}
The initial proposal hypothesized that the RL agent could be trained using an \textit{Offline DRL} approach, learning optimal policies from a fixed, pre-collected dataset of barangay responses. However, preliminary testing revealed that static datasets could not capture the dynamic, cascading consequences of municipal funding allocations over time. 

Because the ABM functions as a highly non-linear Markov Decision Process (MDP)—where a budget allocation at $t_0$ triggers a delayed behavioral shift at $t_1$, which then alters the required budget at $t_2$ due to the "Victim of Success" incentive drain—an offline approach proved fundamentally inadequate. Consequently, the methodology was refined to an \textit{Interactive Online DRL} framework. The PPO agent was coupled directly to the live ABM, treating the simulation as a stochastic environment that provided continuous ``experience replay,'' allowing the AI to organically observe the long-term hysteresis of human behavior \citep{TianReview2024}.
