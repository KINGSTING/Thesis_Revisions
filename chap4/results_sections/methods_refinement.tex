...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{images/non-heuristic_vs_heuristic.png}
...
\begin{enumerate}
    \item \textbf{Algorithm 1 (Lowest-Compliance First):} The reward function penalized the agent based purely on the barangay with the lowest absolute percentage of compliance, regardless of its household population size. The heuristic guided the agent to funnel the largest budget allocations to the worst-performing barangay.
    \item \textbf{Algorithm 2 (Density-Weighted):} The reward function factored in both low compliance and high household population density. The theoretical assumption was that targeting densely populated areas would maximize the raw number of compliant households per budget allocation, thereby optimizing overall efficiency.
\end{enumerate}

Empirical trial tests revealed a significant flaw in Algorithm 2, rooted in what reinforcement learning literature identifies as "population density bias" \citep{Mehrabi2021}.  Because the DRL agent's underlying objective was to maximize cumulative numerical rewards, it discovered a mathematical loophole. It developed a propensity to disproportionately funnel budget allocations into highly populated urban centers (e.g., Barangay Poblacion) even if their compliance rates were only slightly below optimal levels. For the AI, shifting a massive population by merely 2\% to 3\% yielded a much higher absolute numerical reward than completely fixing a sparsely populated rural barangay that was failing at a dismal 10\% compliance rate. 

Consequently, Algorithm 2 generated severe systemic instability. The agent would continuously dump resources into high-population areas to harvest "cheap" marginal rewards, while less-dense rural barangays languished in a "Collapse Zone" (10\% to 40\% compliance) for hundreds of steps because they did not offer enough mathematical incentive to the agent. This mirrors findings in algorithmic public policy where optimizing purely for aggregate efficiency often reinforces systemic inequities, favoring high-density groups while leaving minority outliers in a persistent state of collapse \citep{Alphonso2024}. Because these neglected, low-population barangays were never addressed, they acted as systemic bottlenecks, permanently dragging down the municipality's Global Average Compliance.

To resolve this mathematical exploit, the HuDRL framework ultimately adopted the heuristic defined in Algorithm 1. By completely blinding the agent to household population density and forcing it to strictly target the lowest percentage compliance, the heuristic drove the AI to perform systematic "bottleneck resolution" \citep{TianReview2024}.  This aligns with the \textit{Maximin} equity principle in governance (maximizing the minimum), compelling the DRL agent to raise the "floor" of the entire system. Instead of chasing high-density rewards, the agent is forced into a \textit{Sequential Saturation} strategy—methodically stabilizing the absolute weakest link in the municipality, regardless of how small its population is, before moving to the next. The empirical validation of this chosen policy, and how it achieves complete municipal stabilization, is evaluated in the subsequent Policy Comparison section.

\subsection{Addressing Behavioral Volatility}

The second major deviation from the initial proposal involved the behavioral stability of the Household Agents. Early simulation runs exhibited a phenomenon of ``Instantaneous Collapse,'' where compliance rates in a barangay would plummet from 90\% to 0\% within a single quarter immediately after the Local Government Unit (LGU) withdrew financial support.
...
\begin{itemize}
    \item \textbf{The Tipping Point (70\%) and Deterministic Lock:} It was observed that once a barangay achieved a compliance rate of 70\%, the behavior transitioned from ``incentive-driven'' to ``norm-driven.'' Consequently, the model applies a deterministic hard lock on the Subjective Norm ($w_{sn}$) weight, fixing it at a high sustained value rather than utilizing a flattened dynamic decay curve. This architectural choice is deliberately designed to capture the non-linear phase transitions characteristic of social conventions. As established by threshold models of collective behavior \citep{Granovetter1978, Centola2018}, social tipping points do not manifest as gradual erosions; rather, once a critical mass is achieved, the behavior rapidly institutionalizes into a self-sustaining equilibrium. Implementing a continuous decay curve would falsely imply that social norms inherently erode without constant external fiscal energy. This contradicts the ``ratchet effect'' observed in pro-environmental behaviors, where established communal norms actively resist degradation and act as a permanent behavioral shield \citep{Andre2021, Nyborg2024}.
    
    \item \textbf{The Social Norm Floor (40\%):} To prevent total behavioral collapse during the Deep Reinforcement Learning (DRL) agent's exploration phase, a ``floor'' was implemented. Even if enforcement ceased entirely, the strong social pressure from neighbors prevented compliance from dropping below 40\%. This represents the residual collective memory and the internalized ``moral obligation'' identified in recent waste management studies \citep{Zheng2022}. This mechanism ensures that the model accounts for cultural inertia, preventing sociologically unrealistic swings in household behavior.
\end{itemize}

This refinement was crucial for the success of the Comparative Analysis, as it allowed the HuDRL agent to execute the ``Sequential Saturation'' strategy---concentrating capital in one barangay before moving to another---without the first barangay collapsing behind it, thereby reflecting a more sustainable and mathematically sound governance model \citep{Abdallah2021}.