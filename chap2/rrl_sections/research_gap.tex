...
\begin{table}[htbp]
    \centering
    \caption{Thematic Comparison of Related Literature}
    \label{tab:lit_gap_analysis}
    \small
    \begin{tabularx}{\textwidth}{L{3.2cm} c c c L{5.2cm}}
        \toprule
        \textbf{Category} & \textbf{ABM} & \textbf{DRL} & \textbf{Policy} & \textbf{Gap / Limitation} \\
        \midrule
        
        % ROW 1
        Descriptive Studies (\citet{Abordo2025}) & \xmark & \xmark & \xmark & Audited past compliance but could not predict future policy outcomes. \\ 
        \midrule
    
        % ROW 2
        Traditional ABM (\citet{Ceschi2021}) & \cmark & \xmark & \cmark & Relied on manual scenario testing; lacked autonomous AI optimization. \\ 
        \midrule
    
        % ROW 3
        Logistics Optimization (\citet{Akbarpour2021}) & \xmark & \xmark & \xmark & Optimized fleet routes, ignoring complex household behavior. \\ 
        \midrule
    
        % ROW 4
        AI \& RL (Hardware) (\citet{Jimenez2025}) & \xmark & \cmark & \xmark & Focused on smart bins and robotics rather than governance policy. \\ 
        \midrule
    
        % ROW 5
        Qualitative Reviews (\citet{Santos2025}) & \xmark & \xmark & \xmark & Offered no quantitative mechanism to optimize specific policy parameters. \\ 
        \midrule
    
        % ROW 6
        Proposed Study (Bansao \& Lumingkit) & \cmark & \cmark & \cmark & First study to use ABM-DRL to optimize Philippine LGU ordinance parameters. \\ 
        \bottomrule
    \end{tabularx}
\end{table}
