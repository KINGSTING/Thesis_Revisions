...
However, the DRL agent identified a critical ``Tipping Point'' (approximately 70\% compliance) where social influence flips from a barrier to a facilitator \citep{Centola2018}. Once this threshold is crossed, the behavior becomes self-reinforcing. This explains the ``Graduation Effect'' observed in barangays like Mati and Binuni, which maintained 100\% compliance from Quarter 4 through Quarter 12 even after the agent reduced funding to near-zero ``Maintenance Mode.'' This validates the \textit{Threshold Hypothesis} in Philippine governance, where collective action becomes sustainable only after a critical mass internalizes the mandate, allowing the LGU to redirect scarce resources elsewhere \citep{Nishimura2022}.
