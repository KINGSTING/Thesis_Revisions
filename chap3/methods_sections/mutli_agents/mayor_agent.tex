...
\begin{enumerate}
    ...
    \item \textbf{Macro-Enforcement (LGU Strike Force) and Object Destruction:} To execute localized crackdowns, enforcement funds are translated into elite ``Strike Force'' personnel. Unlike local Tanods, these agents are hired as temporary shock troops. Assuming the Region X minimum daily wage ($W_{Daily} = \text{\textpeso}400$) and a concentrated 30-day deployment window, the municipal enforcement cost ($C_{\mathrm{Enf}}$) is linearly tied to human capital:
    \begin{equation}
