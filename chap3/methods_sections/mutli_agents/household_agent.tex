...
\begin{itemize}
    \item \textbf{Stage 1: Bayanihan Activation (The 40\% Floor):} Triggered when barangay compliance surpasses 40\%, representing an emergent community spirit that slows the erosion of positive norms, ensuring perceived social pressure does not drop to zero \citep{Nishimura2022}.
    \item \textbf{Stage 2: Norm Lock-in (The 70\% Tipping Point):} Occurs when compliance exceeds 70\%, representing deep cultural habituation where segregation becomes the default state \citep{Jimenez2025}. This reflects ``critical mass'' theory, where minority behaviors cascade into majority conventions \citep{Centola2018, Nyborg2024}. 
\end{itemize}

Mathematically, this damping effect is expressed as a piecewise function:
