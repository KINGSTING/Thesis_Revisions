...
In this formulation, $w_A(t)$ represents the temporal evolution of the agent's internal valuation, functioning as a decay model that increases in response to educational campaigns and decays stochastically in their absence to simulate ``public forgetting'' \citep{Trushna2024}. The social component, $SN_{\mathrm{local}}$, is an endogenous variable derived from the observed compliance rate of the agent's immediate spatial neighborhood \citep{Liao2024}. To account for inherent uncertainty in human decision-making, $\epsilon$ introduces Gaussian stochastic noise ($\mu=0, \sigma=0.05$), ensuring the model reflects real-world behavioral variance \citep{Subedi2025}.
...
To achieve this, the model implements a ``Social Norm Shield'' governed by localized compliance thresholds, effectively shifting the behavioral driver from transient attitudes to structural peer pressure \citep{Andre2021}. This shield operates through two distinct programmatic stages:

\begin{itemize}
    \item \textbf{Stage 1: Bayanihan Activation (The 40\% Floor):} Triggered when barangay compliance surpasses 40\%, representing an emergent community spirit. To prevent total mathematical collapse during the DRL agent's early exploration phase, an absolute floor is implemented. Even if enforcement ceases entirely, spatial peer pressure prevents compliance from dropping below this line. This simulates the internalized ``moral obligation'' and residual cultural memory identified in regional waste management studies \citep{Nishimura2022}.
    \item \textbf{Stage 2: Norm Lock-in (The 70\% Tipping Point):} Once a localized node breaches the 70\% compliance threshold, the behavior transitions from incentive-driven to norm-driven. The simulation executes a deterministic hard-lock on the Subjective Norm weight ($w_{sn} = 0.90$), preventing further dynamic decay and simulating cultural hysteresis. This reflects ``critical mass'' theory, where minority behaviors cascade into majority conventions that actively resist degradation \citep{Centola2018, Nyborg2024}.
\end{itemize}

Mathematically, this mechanism replaces standard dynamic decay with a conditional piecewise lock on the Subjective Norm variable:

\begin{equation}
    w_{sn}(t) = 
    \begin{cases} 
        0.90 & \text{if } \text{Compliance}_{\mathrm{local}} \ge 0.70 \text{ (Stage 2: Norm Lock-in)} \\
        \max(w_{sn}(t_0), 0.40) & \text{if } \text{Compliance}_{\mathrm{local}} \ge 0.40 \text{ (Stage 1: Bayanihan Floor)} \\
        w_{sn}(t_0) & \text{otherwise (Weak Norms / Standard Behavior)}
    \end{cases}
\end{equation}
\addequation{Deterministic Hard-Lock for Subjective Norms}
