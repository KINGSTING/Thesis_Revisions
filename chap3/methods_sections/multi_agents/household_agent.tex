...
Upon instantiation, each agent is assigned a fixed demographic parameter (Income Level) and a continuously updating psychological state vector $S_{t} = \langle A, SN, PBC \rangle$. To prevent synthetic uniformity, the initial state vector is populated using stochastic Gaussian sampling bounded by the agent's assigned behavioral profile. 

\paragraph{Notation Legend:}
The notation \(X \sim \mathcal{U}(\alpha, \beta)\) indicates that variable \(X\) is randomly sampled from a continuous uniform distribution bounded between \(\alpha\) (the minimum) and \(\beta\) (the maximum). Thus, \(A \sim \mathcal{U}(0.75, 0.90)\) means the agent's initial attitude score \(A\) is drawn uniformly at random from the interval \([0.75, 0.90]\). Similarly, \(W_i \sim \mathcal{N}(\mu, \sigma^2)\) indicates sampling from a normal distribution with mean \(\mu\) and variance \(\sigma^2\). 

Using these distributions, agents initialized as ``Compliant'' derive their base attitude from \(A \sim \mathcal{U}(0.75, 0.90)\), whereas ``Non-Compliant'' agents are seeded from \(A \sim \mathcal{U}(0.10, 0.40)\).

While the \texttt{BEHAVIOR\_PROFILES} provide the foundational centroids for each sector, individual agents are instantiated via stochastic Gaussian sampling to ensure population heterogeneity. Specifically, the weight parameters ($w_a, w_{sn}, w_{pbc}$) of a Household Agent are derived from $\mathcal{N}(\mu, \sigma^2)$, where $\mu$ is the fixed barangay config value and $\sigma = 0.05$. This prevents mathematical uniformity and allows the simulation to capture the localized variance inherent in human social systems.
...
\end{equation}
\addequation{The Local Compliance Ratio Equation}
...
% -------------------------------------------------------
% BEGIN: ALIGNED MARGIN FIGURE FOR ALGORITHM 2
% -------------------------------------------------------
\begin{figure}[htbp]
    \centering
    \vspace{6pt} 
    \begin{tikzpicture}[>=stealth, thick, font=\small]
        % ================= Algorithm (Center) =================
        \node[draw, rounded corners, fill=gray!10, text width=2.8cm, align=center, minimum height=2cm, line width=1pt, inner sep=8pt] (algo) at (0,0) {\textbf{Algorithm 2}\\[0.1cm] \footnotesize Daily Step Logic};

        % ================= Inputs (Left Side) =================
        \node[draw, rounded corners, fill=blue!5, text width=4.2cm, align=center, line width=0.8pt, inner sep=5pt] (inputBox) at (-5.0cm, 0) {\textbf{Algorithm 2 Inputs}\\[0.1cm] \footnotesize Household Agent Object \(HH_i(t)\)};
        
        % Display the Python dictionary contents
        \node[anchor=north] at (inputBox.south) {
            \resizebox{3.8cm}{!}{
                \begin{tabular}{|l|l|}
                    \hline
                    \textbf{Variable} & \textbf{Value / Type} \\
                    \hline
                    \texttt{income\_level} & \(\in \{\)\texttt{'Low'}, \texttt{'Mid'}, \texttt{'High'}\(\}\) \\
                    \hline
                    \texttt{attitude} & \(A_i(t) \in [0, 1]\) (Float) \\
                    \hline
                    \texttt{w\_SN} & \(\in [0, 1]\) (Float) \\
                    \hline
                    \texttt{C\_effort} & \(\in [0.1, 1.0]\) (Float) \\
                    \hline
                \end{tabular}
            }
        };
        ...
        % ================= Outputs (Right Side) =================
        % SHIFTED LEFT: Moved from 5.5cm to 5.0cm to align strictly with the right margin.
        \node[draw, rounded corners, fill=green!5, text width=3.5cm, align=center, line width=0.8pt, inner sep=8pt] (outputBox) at (5.0cm, 0) {\textbf{Decision:}\\[0.1cm] \footnotesize Segregate \\[0.1cm] \footnotesize Do Not Segregate};

        % ================= Arrows =================
        % Arrows automatically adjust to the new box positions
        \draw[->, line width=1pt, shorten >=3pt] (inputBox.east) -- (algo.west);
        \draw[->, line width=1pt, shorten >=3pt] (algo.east) -- (outputBox.west);
    \end{tikzpicture}
    \vspace{6pt} 
    \caption{Data Representation: Inputs, daily decision logic, and outputs of Algorithm 2.}
    \label{fig:data_rep_alg2}
\end{figure}
% -------------------------------------------------------
% END: ALIGNED MARGIN FIGURE FOR ALGORITHM 2
% -------------------------------------------------------

\paragraph{Data Representation: Algorithm 2 Input and Output}
Algorithm 2 governs the daily decision-making process of each independent \texttt{HouseholdAgent} at a given time step \(t\). The algorithm receives a structured Python dictionary object, denoted as \(HH_i(t)\), containing four state variables that define the agent's current socio-psychological profile. The \texttt{income\_level} parameter is a categorical variable (\(\in \{\text{'Low'}, \text{'Mid'}, \text{'High'}\}\)) that scales the agent's financial sensitivity (\(\gamma\)) to municipal fines and monetary incentives. The \texttt{attitude} parameter is a floating-point value \(A_i(t) \in [0, 1]\), dynamically updated by the effects of IEC campaigns and subject to natural decay. The \texttt{w\_SN} value determines the agent's susceptibility to peer pressure and community social norms, while \texttt{C\_effort} represents the static physical friction of segregation. Utilizing these inputs, Algorithm 2 executes its step logic: it first evaluates the local segregation rate using a Moore neighborhood query to enforce the ``Habit Shield,'' then calculates the ``Net Effort'' by balancing physical hassle against the financial impact of incentives and penalties. Finally, it introduces a 1\% stochastic human error check and outputs the binary decision: \texttt{Segregate} or \texttt{Do Not Segregate}, which updates the agent's state for the current tick.

\begin{algorithm}[H]
\caption{The Household's Daily Decision Process (To Segregate or Not?)}
\label{alg:household_decision_english}
\begin{algorithmic}[1]
\State \textbf{Input:} Household Income, Baseline Motivation, Peer Pressure sensitivity, and Base Effort (see Figure \ref{fig:data_rep_alg2}).
\Statex
...
By calculating the \(S_1\) and \(S_T\) indices, the analysis isolates which specific computational parameters most heavily dictate systemic outcomes. Identifying these highly sensitive parameters verifies whether the emergent behavioral dynamics of the ABM align with established psychological theories of waste segregation, ensuring the subsequent Deep Reinforcement Learning (DRL) agent is optimizing against a structurally sound and ecologically valid virtual population \citep{TianReview2024}.